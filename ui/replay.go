...
func RunReplay(inputFilename string, logFilename string) error {
	...
	insertEditor, loadCtx, finalizer, err := makeInsertEditor(ctx, inputFilename, "", 20, 20)
	if err != nil {
		return err
	}
	...
	go func() {
		for {
			...
			case <-ctx.Done():
				return
			case view := <-insertEditor.Update(): // consume view
